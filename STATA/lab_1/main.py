...
# df['Years_in_Company'] = (pd.Timestamp.now() - df['Hire_Date']).dt.days / 365
...
# performance_salary_corr = df[['Performance_Rating', 'Salary_INR']].corr()
# print(f"\n9. Корреляция между рейтингом и зарплатой: {performance_salary_corr}")
...
if __name__=="__main__":
    ...
    # print(df['Department'],'\n',df['Salary_INR'])
    # print(df[df['Salary_INR'] > 1000000].sort_values('Salary_INR', ascending=False))
    # print(df[df['Salary_INR'] > 1000000]['Job_Title'])
    # print(df['Department'].unique())
    # df = df.rename(columns={'Salary_INR': 'Salary'})
    # print(df['Salary']) 
    # print(df.groupby('Department')['Salary_INR'].mean())
    # print(df.groupby('Department')['Salary_INR'].max())
    # print(df.groupby('Department')['Salary_INR'].min())
    # print(df.groupby('Department')['Salary_INR'].mean())
    # print(df[df['Experience_Years'] > 5])
    # print(df.columns)
    # print(df[(df['Salary_INR'] > 1000000) & (df['Department'] == 'HR')])
    # print(df['Salary_INR'].median())
    # print((df['Salary_INR']==0).sum)
    #print(df.groupby('')['Full Name'].agg(count='count', mean='mean', median='median'))
    print(df.query("Job_Title == 'Software Engineer'")["Salary_INR"].mean())
    df['Salary'] = df['Salary_INR'] * 0.011
    print(df['Salary'])