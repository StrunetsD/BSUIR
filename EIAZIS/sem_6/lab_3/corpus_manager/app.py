...
def process_file_direct(file_stream, filename, title, author, genre, year):
    ...
    try:
        ...
        # Короткие демо-страницы (например tests/test_tiny_note.html) дают ~45 символов — порог не должен быть выше реального текста из одного <p>.
        if not text_value or len(text_value) < 35:
