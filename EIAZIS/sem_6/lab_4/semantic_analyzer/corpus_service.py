...
from syntax_parser import build_nested_tree, get_nlp, parse_dependencies_spacy
...
def _build_semantic_entries(tokens):
    semantic_entries = []
    seen = set()
    ...
    for token in tokens:
        ...
        lemma = token.get('lemma') or token.get('text', '').lower()
        synsets = wordnet.synsets(lemma, pos=wn_pos)
        if not synsets:
            continue

        synset = synsets[0]
        key = (lemma, synset.name())
        if key in seen:
            continue
        seen.add(key)
    ...
    return semantic_entries[:8]


def _extract_named_entities(sentence_text):
    nlp = get_nlp()
    if nlp is None:
        return []
...
def analyze_sentence_syntax(sentence_text):
    ...
    if spacy_result:
        ...
        direct_object = next(
            (
                row['text'] for row in spacy_result['tokens']
                if row.get('dep') in {'dobj', 'obj', 'pobj', 'attr'}
            ),
            None
        )
        dependency_parse = {
        ...
        direct_object = next(
            (
                row['text'] for row in dependency_rows
                if row.get('relation') in {'obj', 'dobj'}
            ),
            None
        )
        heuristic_tokens = [
    ...
    named_entities = _extract_named_entities(sentence_text)
    semantic_entries = _build_semantic_entries(dependency_parse['tokens'])
    named_entity_summary = (
        ', '.join(f"{item['text']} ({item['label']})" for item in named_entities)
        if named_entities else 'not found'
    )
    semantic_summary = (
        ', '.join(item['sense'] for item in semantic_entries)
        if semantic_entries else 'not found'
    )

    summary = '\n'.join([
        f"Clause core: subject - {subject or 'not found'}; predicate - {predicate or 'not found'}.",
        f"Object: {direct_object or 'not found'}.",
        ...
        f"Named entities: {named_entity_summary}.",
        f"Semantic concepts: {semantic_summary}.",
    ])
