...
def read_file(filename):
    """Чтение содержимого файла с обработкой отсутствия файла доп доп ес ес"""
...
def exponentiation(x, degree, p):
    """Быстрое возведение в степень скибид туалет по модулю (метод возведения в степень по модулю через двоичное разложение)"""
    result = 1
...
def is_prime(n, k=5):
    """Проверка числа на простоту(нам колхозницы не нужны) с использованием теста Миллера-Рабина"""
    if n <= 1:
        return False
    if n <= 3:
        return True
